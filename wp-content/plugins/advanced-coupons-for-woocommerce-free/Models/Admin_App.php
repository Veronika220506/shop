...
class Admin_App implements Model_Interface, Initializable_Interface
{
    ...
    public function register_react_scripts($screen, $post_type)
    {
        ...
        if (defined('ACFW_SETTINGS_APP_URL') && ACFW_SETTINGS_APP_URL) {

            wp_enqueue_script('acfwp-edit-coupon-app-bundle', ACFW_SETTINGS_APP_URL . '/static/js/bundle.js', array('wp-api'), $this->_constants->VERSION, true);
            wp_enqueue_script('acfwp-edit-coupon-app-vendor', ACFW_SETTINGS_APP_URL . '/static/js/vendors~main.chunk.js', array('wp-api'), $this->_constants->VERSION, true);
            wp_enqueue_script('acfwp-edit-coupon-app-main', ACFW_SETTINGS_APP_URL . '/static/js/main.chunk.js', array('wp-api'), $this->_constants->VERSION, true);
            ...
            $app_js_url   = $this->_constants->JS_ROOT_URL() . 'app/settings/build/static/js/';
            $app_css_url  = $this->_constants->JS_ROOT_URL() . 'app/settings/build/static/css/';
                ...
                if ($js_files = \scandir($app_js_path)) {
                    foreach ($js_files as $key => $js_file) {
                        if (strpos($js_file, '.js') !== false && strpos($js_file, '.js.map') === false && strpos($js_file, '.js.LICENSE.txt') === false) {
                            ...
                            wp_enqueue_script($handle, $app_js_url . $js_file, array(), Plugin_Constants::VERSION, true);
                        }
                    }
                }
            }
