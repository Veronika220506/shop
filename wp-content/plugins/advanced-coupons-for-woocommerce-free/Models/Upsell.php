...
class Upsell implements Model_Interface, Initializable_Interface
{
    ...
    public function register_upsell_panels($panels)
    {
        ...
        foreach ($panels as $key => $panel) {
            ...
            if ('acfw_bogo_deals' === $key) {
                $filtered_panels['acfw_add_products'] = array(
                    'label'  => __('Add Products (Premium)', 'advanced-coupons-for-woocommerce-free'),
                    'target' => 'acfw_add_products',
                    'class'  => '',
                );

                $filtered_panels['acfw_scheduler'] = array(
            ...
            if ('acfw_role_restrictions' === $key) {
                $filtered_panels['acfw_payment_method_restrictions'] = array(
            ...
            // add panels after url coupons
            if ('acfw_url_coupon' === $key) {
                $filtered_panels['acfw_shipping_overrides'] = array(
                    'label'  => __('Shipping Overrides (Premium)', 'advanced-coupons-for-woocommerce-free'),
                    'target' => 'acfw_shipping_overrides',
                    'class'  => '',
                );

                $filtered_panels['acfw_apply_notification'] = array(
    ...
    public function enqueue_upgrade_settings_scripts($screen, $post_type)
    {
        ...
        if ('woocommerce_page_wc-admin' === $screen->id || 'edit-shop_coupon' === $screen->id) {
            ...
            wp_enqueue_script('acfw-wc-admin', $this->_constants->JS_ROOT_URL() . 'app/wc-admin/dist/acfw-wc-admin.js', array('wc-admin-app', 'wc-components'), Plugin_Constants::VERSION, true);
